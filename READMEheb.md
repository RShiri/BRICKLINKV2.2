...
├── scan_superheroes.py       # סורק אוטונומי של מיניפיגים Marvel/DC
├── scan_catalog.py           # זחלן עץ קטלוג BrickLink
├── scan_all_minifigs.py      # סורק ערכות נושא אוניברסלי (15+ ערכות)
├── pages/
...
### 🕷️ סוכני כריית נתונים אוטונומיים
הפרויקט כולל סקריפטים עצמאיים לאיסוף נתונים המוני:

- **`scan_superheroes.py`**: סורק חכם שעובר על `sh0001`-`sh9999`, מזהה פערים, ובונה מסד נתונים שלם של Marvel/DC.
- **`scan_catalog.py`**: זחלן שממפה את כל עץ הקטגוריות של BrickLink כדי לגלות ערכות חדשות אוטומטית.
- **`scan_all_minifigs.py`**: סורק אוניברסלי שמטפל ב-15+ ערכות נושא (Star Wars, Harry Potter, Ninjago) עם זיהוי פערים חכם.

*הרץ אותם ברקע כדי להזין את הדשבורד בנתוני שוק טריים.*

**דוגמת שימוש:**
```bash
# סרוק את כל מיניפיגי גיבורי העל (sh0001-sh9999)
python scan_superheroes.py

# גלה ערכות LEGO חדשות מקטלוג BrickLink
python scan_catalog.py

# סרוק טווחי נושא ספציפיים
python scan_all_minifigs.py
```

### 🔐 בקרת גישה מבוססת תפקידים
- **מצב משתמש**: גישה לקריאה בלבד לניתוח שוק ומסדי נתונים ציבוריים.
- **מצב מנהל**: אזור מוגן בסיסמה (`7399`) עם הרשאות כתיבה:
  - מחיקת פריטים ממסד הנתונים
  - ניהול תיקים ספציפיים (אוסף של רם / אוסף של אודי)
  - גישה לעורך נתונים גולמיים
  - יכולות כפיית סריקה מחדש

### 📱 עיצוב Mobile-First
הזרקת CSS מותאמת אישית מבטיחה שהדשבורד מגיב באופן מלא בסמארטפונים, מותאם לבדיקת מחירים תוך כדי ציד בחנויות פיזיות. הממשק מסתגל בצורה חלקה למסכים קטנים עם בקרות ידידותיות למגע.
